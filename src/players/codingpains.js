...
var codingpains = {
  ...
  ai: (playerState, enemiesStates, gameEnvironment) => {
    ...
    if (playerState.ammo) {
      ...
      if (turnMovement && isMovementSafe(turnMovement, playerState, enemiesStates)) {
        console.log('Turn ', turnMovement);
        return turnMovement;
      }

      closestEnemy = getClosestEnemy(playerState, enemiesStates);
      closestAmmo = getClosestAmmo(playerState, gameEnvironment);

      chaseMovement = chaseEnemy(playerState, enemiesStates);
      if (chaseMovement && isMovementSafe(chaseMovement, playerState, enemiesStates)) {
        console.log('Chase this fucker', chaseMovement);
        return chaseMovement;
      }
    ...
    console.log("No ammo go find some ammo");
    ammoMovement = shouldMoveForAmmo(playerState, gameEnvironment);
    if (ammoMovement) {
      return ammoMovement;
    }
